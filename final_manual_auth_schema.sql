-- FINAL Manual Authentication Schema for BRAC University Portal
-- This completely replaces Supabase built-in auth with custom user management
...
-- Step 4: Drop existing constraints and recreate tables for clean migration
-- Only drop foreign key constraints in public schema (we can't modify auth schema)
do $$ 
declare
    r record;
begin
    -- Drop foreign key constraints in public schema that reference auth.users
    for r in (
...
exception
    when others then
        -- If auth.users doesn't exist or we can't access it, that's fine
        null;
end $$;

-- Drop existing tables that we'll recreate
drop table if exists public.user_roles cascade;
drop table if exists public.profiles cascade;
drop table if exists public.user_sessions cascade;

-- Step 5: Create the main users table
...
-- Step 9: Create authentication functions
...
-- Step 10: Create user management functions
...
-- Step 11: Create authentication function
...
-- Step 12: Create session validation function
...
-- Step 13: Create logout function
...
-- Step 14: Create session context functions for RLS
...
-- Step 15: Create role helper function
...
-- Step 16: Update existing table foreign keys to reference new users table
do $$ 
declare
    table_name text;
...
-- Step 17: Create remaining tables if they don't exist (from original schema)
...
-- Step 19: Drop all existing policies first
do $$ 
declare
    pol record;
begin
...
-- Step 22: Storage setup (if using Supabase storage)
...
begin
    ...
    drop policy if exists "Resource owners can access resources" on storage.objects;
    drop policy if exists "Authenticated users can upload resources" on storage.objects;
    drop policy if exists "Owners can read their resources" on storage.objects;
    drop policy if exists "Admins can read all resources" on storage.objects;
    drop policy if exists "Owners can upload resources" on storage.objects;
    drop policy if exists "Owners/admins can update resources" on storage.objects;
    drop policy if exists "Owners/admins can delete resources" on storage.objects;
    ...
    create policy "Resource owners can access resources" on storage.objects 
...
exception when others then
    -- Storage policies might not be supported in all Supabase versions
    null;
end $$;

-- Step 24: Create initial admin user (uncomment and modify as needed)
...
-- Step 25: Clean up old sessions periodically (optional - you can set up a cron job for this)
...
select 'Manual authentication system deployed successfully!' as status,
