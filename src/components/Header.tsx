...
const Header = () => {
  ...
  const authenticatedNavItems = [
    ...
    ...(user?.role !== 'admin' ? [{ name: "Profile", href: "/profile", icon: User }] : []),
    ...(user?.role === 'admin' ? [{ name: "Admin", href: "/admin", icon: Shield }] : []),
    ...(user?.role === 'faculty' ? [{ name: "Manage Events", href: "/events", icon: Calendar }] : []),
  ];
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center space-x-3 hover:opacity-90 transition-opacity">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground hidden sm:block">Student Activity Management Portal</p>
            </div>
          </Link>
